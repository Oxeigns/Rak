...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not update.effective_message or not update.effective_chat:
        return
    if not await enforce_force_join(update, context, settings.force_channel_id):
        await update.effective_message.reply_text(
            styled_card("ᴊᴏɪɴ ʀᴇǫᴜɪʀᴇᴅ", "ᴘᴇʜʟᴇ ᴄʜᴀɴɴᴇʟ ᴊᴏɪɴ ᴋᴀʀᴏ, ᴘʜɪʀ ᴠᴇʀɪғʏ ᴅᴀʙᴀᴏ."),
...
async def panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await is_admin(update, context):
        await update.effective_message.reply_text(styled_card("🚫 ɴᴏ ᴀᴄᴄᴇss", "ʏᴇʜ ᴘᴀɴᴇʟ ꜱɪʀꜰ ᴀᴅᴍɪɴ ᴋᴇ ʟɪʏᴇ ʜᴀɪ."))
        return
    await update.effective_message.reply_text(
...
async def set_delay_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not update.effective_chat or not update.effective_message:
        return
    if not await is_admin(update, context):
        await update.effective_message.reply_text(styled_card("🚫 ɴᴏ ᴀᴄᴄᴇss", "ꜱɪʀꜰ ᴀᴅᴍɪɴ ᴅᴇʟᴀʏ ʙᴀᴅᴀʟ ꜱᴀᴋᴛᴀ ʜᴀɪ."))
        return
    if not context.args or not context.args[0].isdigit():
        await update.effective_message.reply_text(styled_card("ɪɴᴠᴀʟɪᴅ", "ᴜsᴇ: /setdelay 1-86400"))
        return
    seconds = int(context.args[0])
    if seconds < MIN_DELETE_DELAY_SECONDS or seconds > MAX_DELETE_DELAY_SECONDS:
        await update.effective_message.reply_text(styled_card("ɪɴᴠᴀʟɪᴅ", "ʀᴀɴɢᴇ 1 sᴇ 86400 sᴇᴄ ᴛᴀᴋ ʀᴀᴋʜᴏ."))
        return
    await context.application.bot_data["store"].set_delete_delay(update.effective_chat.id, seconds)
    await update.effective_message.reply_text(styled_card("✓ sᴀᴠᴇᴅ", f"ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ ᴀʙ {seconds}s ʜᴏ ɢᴀʏᴀ."))
