...
class RuntimeStore:
    ...
    def _execute_with_retry(self, func, *args, max_retries: int = 3):
    ...
    async def get_delete_delay(self, chat_id: int) -> int:
        ...
        async with self._lock:
            return await asyncio.to_thread(self._execute_with_retry, self._get_delete_delay_sync, chat_id)
    ...
    async def set_delete_delay(self, chat_id: int, seconds: int) -> None:
        ...
        async with self._lock:
            await asyncio.to_thread(self._execute_with_retry, self._set_delete_delay_sync, chat_id, seconds)
    ...
    async def increment_warning(self, chat_id: int, user_id: int) -> int:
        ...
        async with self._lock:
            return await asyncio.to_thread(self._execute_with_retry, self._increment_warning_sync, chat_id, user_id)
    ...
    async def reset_warning(self, chat_id: int, user_id: int) -> None:
        ...
        async with self._lock:
            await asyncio.to_thread(self._execute_with_retry, self._reset_warning_sync, chat_id, user_id)
    ...
    async def track_chat(self, chat_id: int, chat_type: str) -> None:
        ...
        async with self._lock:
            await asyncio.to_thread(self._execute_with_retry, self._track_chat_sync, chat_id, chat_type)
    ...
    async def get_all_chats(self) -> list[tuple[int, str]]:
        ...
        async with self._lock:
            return await asyncio.to_thread(self._execute_with_retry, self._get_all_chats_sync)
    ...
    async def get_total_warnings(self) -> int:
        ...
        async with self._lock:
            return await asyncio.to_thread(self._execute_with_retry, self._get_total_warnings_sync)
